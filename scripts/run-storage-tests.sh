...
#
# Runs the 17 Beta Foundation V1 Storage/Auth integration tests
# against the local disposable Supabase instance in Stage 3 state.
#
# Pipeline:
#   1. Safety checks (guards against production contact)
#   2. Discover JWT keys from local Kong container (no hardcoded secrets)
#   3. Reset local disposable DB to D11 baseline (clears all user/row data)
#   4. Apply beta_foundation_v1.sql (historical migration)
#   5. Apply corrective migration (20260729120001_...)
#   6. Run storage.integration.test.ts with RUN_DATABASE_TESTS=1
#
# Rebuilding Stage 3 from D11 guarantees no stale auth users with FK-constrained
# rows, so the storage test's beforeAll cleanup finds a clean slate.
#
# SAFETY: Only touches the local disposable Supabase Docker instance.
# Never contacts the hosted production project.
# JWT keys are discovered at runtime — no secrets hardcoded here.
#
# Prerequisites:
...
STORAGE_TEST="validation/beta-foundation-v1/storage/storage.integration.test.ts"
...
safety_checks() {
  ...
  if [[ "$E2E_SUPABASE_URL" == *"ujwfkhvmpdmgjausnbre"* ]]; then
    die "Production Supabase project ref detected in E2E_SUPABASE_URL"
  fi
  if [[ "$E2E_SUPABASE_URL" == *".supabase.co"* ]]; then
    die "Hosted Supabase URL detected in E2E_SUPABASE_URL"
  fi
  ...
  if [[ ! -f "$ROOT_DIR/$STORAGE_TEST" ]]; then
    die "Storage test not found: $ROOT_DIR/$STORAGE_TEST"
  fi

  # Verify endpoints resolve to loopback
  local direct_host
  direct_host=$(echo "$DIRECT_URL" | sed -E 's|.*@([^:/]+).*|\1|')
  if [[ "$direct_host" != "127.0.0.1" && "$direct_host" != "localhost" ]]; then
    die "DIRECT_URL host '$direct_host' is not loopback (127.0.0.1 or localhost)"
  fi

  pass "All production safety checks passed."
  log "  DIRECT_URL:       $DIRECT_URL (loopback: $direct_host)"
  log "  E2E_SUPABASE_URL: $E2E_SUPABASE_URL"
  log "  SUPABASE_ACCESS_TOKEN: ABSENT"
  log "  Production ref ujwfkhvmpdmgjausnbre: ABSENT"
  log "  Hosted *.supabase.co: ABSENT"
}

# ── DB reset to D11 baseline ───────────────────────────────────────────────────
reset_to_d11() {
  sep
  log "Resetting local DB to D11 baseline (clears all rows including stale test users)..."
...
# ── Run the storage integration test suite ─────────────────────────────────────
run_storage_tests() {
  sep
  log "Running Storage/Auth integration tests: $STORAGE_TEST"
  log "17 tests — RUN_DATABASE_TESTS=1"
...
main() {
  echo
  echo "╔══════════════════════════════════════════════════════════════════╗"
  echo "║     MoLis Beta Foundation V1 — Storage/Auth Integration Tests   ║"
  echo "║     storage.integration.test.ts — 17 tests                      ║"
  echo "╚══════════════════════════════════════════════════════════════════╝"
  echo
  ...
  sep
  log "DB is in Stage 3 (post-corrective) state. Auth users cleared. Running storage tests..."
